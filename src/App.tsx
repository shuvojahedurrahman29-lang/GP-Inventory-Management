...
export default function App() {
  ...
  const processTransaction = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const [staffDoc, prodDoc] = await Promise.all([
        ...
        if (transactionType === 'ISSUE') {
          if (currentProdStock < transactionQty) {
            throw new Error('Insufficient stock in master inventory');
          }
          ...
          // RETURN
          if (holdingIndex === -1 || newStaffHoldings[holdingIndex].qtyHeld < transactionQty) {
            throw new Error('Staff does not hold enough of this product to return');
          }
          newProdStock += transactionQty;
        ...
        transaction.update(prodRef, { totalStock: newProdStock });
  ...
  const generateReceiptPDF = (
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Payment Type: ${payment}`, 140, finalY + 22);
